      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .r-btn-primary {
          ...
          box-shadow: 0 4px 12px rgba(37,99,235,0.28); transition: all 0.15s;
        }
        ...
        .r-select {
          padding: 9px 12px; border: 1.5px solid #e2e8f0; border-radius: 10px;
          font-size: 13px; outline: none; background: #fff; width: 100%;
          transition: border-color 0.15s; cursor: pointer; color: #0f172a;
          box-sizing: border-box;
        }
        ...
        .r-th {
          ...
          letter-spacing: 0.5px; background: #f8fafc; white-space: nowrap;
          cursor: pointer; user-select: none;
        }
        .r-th:first-child { border-radius: 10px 0 0 0; }
        .r-th:last-child  { border-radius: 0 10px 0 0; }
        ...
        @media (max-width: 480px) {
          .r-modal { padding: 20px 16px; border-radius: 14px; }
          .r-page-title { font-size: 19px; }
          .r-main-inner { padding: 0; }
        }

        .rotasi-wrapper { display: flex; min-height: 100vh; min-height: 100dvh; }
        ...
        .rotasi-topbar-page { font-size: 12px; color: #64748b; }
        .rotasi-hamburger {
        ...
        .rotasi-bottomnav-item {
          flex: 1; display: flex; flex-direction: column; align-items: center;
          padding: 8px 4px; text-decoration: none; color: #64748b;
          font-size: 10px; font-weight: 600; gap: 3px; transition: all 0.15s;
        }
        .rotasi-bottomnav-item .bnav-icon { font-size: 20px; line-height: 1; }
        .rotasi-bottomnav-item.active { color: #60a5fa; }
        .rotasi-bottomnav-item.active .bnav-icon { transform: scale(1.1); }

        /* ── DRAWER mobile menu ── */
        .rotasi-drawer-overlay {
          display: none; position: fixed; inset: 0; z-index: 60;
          background: rgba(0,0,0,0.6); backdrop-filter: blur(4px);
        }
        .rotasi-drawer-overlay.open { display: block; }
        .rotasi-drawer {
        ...
        @media (max-width: 768px) {
          ...
          .rotasi-main {
            margin-left: 0; padding: 72px 16px 80px;
          }
        }
      ...
      <div className="rotasi-wrapper">
        ...
        <div className="rotasi-topbar">
          <div>
            ...
            {currentNav && <div className="rotasi-topbar-page">{currentNav.full}</div>}
          </div>
          <button className="rotasi-hamburger" onClick={() => setMenuOpen(true)}>☰</button>
        </div>

        {/* ── Drawer mobile ── */}
        <div className={`rotasi-drawer-overlay${menuOpen ? ' open' : ''}`} onClick={() => setMenuOpen(false)} />
        <div className={`rotasi-drawer${menuOpen ? ' open' : ''}`}>
          <div className="rotasi-drawer-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div className="rotasi-brand-icon" style={{ width: 32, height: 32, borderRadius: 8, fontSize: 16 }}>🔄</div>
              <div>
                <div className="rotasi-brand-text">Rotasi</div>
                <div className="rotasi-brand-sub">Jadwal Terapis</div>
              </div>
            </div>
            <button className="rotasi-drawer-close" onClick={() => setMenuOpen(false)}>✕</button>
          </div>
