...
const RotasiProtectedRoute = ({ children }) => {
  const { user, loading: authLoading } = useAuth();
  const location = useLocation();
  const [status, setStatus] = useState('checking'); // checking | ok | forbidden | unauthenticated
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-4">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 text-center max-w-md w-full">
          ...
          <p className="text-slate-600 mb-6">
            Akun ini tidak terdaftar sebagai admin sistem Rotasi Jadwal Terapis.
          </p>
...
export default RotasiProtectedRoute;