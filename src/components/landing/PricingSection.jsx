...
import { CheckCircle2, Clock, UserCheck, ClipboardList, Target, ShieldCheck, ArrowRight, Star } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Link } from 'react-router-dom';

const valueProps = [
  { icon: Clock, text: "Datang sesuai jam (tanpa antre)" },
  { icon: UserCheck, text: "One-on-one dengan fisioterapis" },
  { icon: ClipboardList, text: "Program berbasis klinis" },
  { icon: Target, text: "Fokus hasil jangka panjang" },
  { icon: ShieldCheck, text: "Nyaman, aman, & profesional" },
];
...
const PricingCard = ({ 
  ...
  const CardWrapper = isBestValue ? motion.div : 'div';
  
  return (
    <div className={`relative h-full flex flex-col ${isBestValue ? 'lg:-mt-4 lg:-mb-4 z-10' : ''}`}>
      <div 
        className={`
          h-full rounded-2xl p-6 flex flex-col transition-all duration-300 relative overflow-hidden
          ${isBestValue 
    ...
    <section className="py-20 bg-slate-50" id="pricing">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-4">Tarif Layanan Kaffah Physiotherapy</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-16">
          {valueProps.map((prop, idx) => (
            <motion.div 
              ...
              className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col items-center text-center gap-3 hover:shadow-md transition-all"
            >
              <div className="bg-blue-50 p-2 rounded-full text-kaffah-blue">
                <prop.icon className="w-5 h-5" />
              </div>
              <p className="text-sm font-medium text-slate-700">{prop.text}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="mt-12 text-center">
            <div className="inline-flex flex-wrap justify-center gap-4 md:gap-8 bg-white px-6 py-4 rounded-full shadow-sm border border-slate-100">
                <div className="flex items-center gap-2 text-sm md:text-base font-medium text-slate-700">
                    <CheckCircle2 className="w-5 h-5 text-kaffah-blue" /> One-on-one physiotherapy
                </div>
                <div className="flex items-center gap-2 text-sm md:text-base font-medium text-slate-700">
                    <CheckCircle2 className="w-5 h-5 text-kaffah-blue" /> Program terapi personal
                </div>
                <div className="flex items-center gap-2 text-sm md:text-base font-medium text-slate-700">
                    <CheckCircle2 className="w-5 h-5 text-kaffah-blue" /> Ditangani fisioterapis berpengalaman
                </div>
            </div>
        </div>
        ...
        <motion.div 
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="mt-20 relative rounded-3xl overflow-hidden bg-gradient-to-r from-kaffah-navy to-blue-600 px-6 py-12 md:px-12 text-center shadow-xl"
        >
            <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]"></div>
            <div className="relative z-10 max-w-2xl mx-auto space-y-6">
                <h3 className="text-2xl md:text-3xl font-bold text-white">
                    Mulai Terapi dengan Lebih Terencana & Hemat
                </h3>
                <p className="text-blue-100 text-lg">
                    Pilih paket yang sesuai dan jadwalkan sesi fisioterapi Anda sekarang.
                </p>
                <Link to="/booking">
