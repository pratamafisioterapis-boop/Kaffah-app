...
import { createAdminExpense, updateAdminExpense, getAccountingSubcategories, getBankAccounts } from '@/lib/api';
import { format } from 'date-fns';
import SearchableSelect from '@/components/ui/searchable-select';

const AdminExpenseForm = ({ onSuccess, onCancel, initialData = null }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.amount || !form.category || !form.sub_category || !form.transaction_date || !form.bank_account_id) {
      toast({ variant: "destructive", title: "Missing Fields", description: "Please fill in all required fields including Sub Category and Bank Account." });
      return;
    }
    ...
    try {
      if (isEditMode) {
        const { error } = await updateAdminExpense(initialData.id, form);
        if (error) throw error;
        ...
        const { error } = await createAdminExpense(payload);
        if (error) throw error;
        toast({ title: "Expense Recorded", description: "Successfully added new expense record." });
      }
  ...
  return (
    <form onSubmit={handleSubmit} className={`space-y-4 ${!isEditMode ? 'bg-white p-6 rounded-xl border border-slate-200' : ''}`}>
      ...
      <div className="space-y-2">
        <Label htmlFor="category">Main Category (Auto-filled)</Label>
        <Input 
          id="category" 
          value={form.category}
          readOnly
