...
import { deleteAdminIncome } from '@/lib/api';
import { useToast } from '@/components/ui/use-toast';
import { format } from 'date-fns';

const AdminIncomeDeleteConfirmationModal = ({ isOpen, onClose, income, onSuccess }) => {
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      const { error } = await deleteAdminIncome(income.id);
      ...
      toast({ variant: "destructive", title: "Gagal", description: error.message || "Gagal menghapus pemasukan." });
    } finally {
      setLoading(false);
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={(val) => !loading && onClose(val)}>
      <DialogContent className="sm:max-w-[450px] bg-white rounded-xl border-0 shadow-xl">
        <DialogHeader className="flex flex-col items-center gap-2 pb-2">
          ...
          <DialogTitle className="text-center text-xl font-bold text-slate-900">Hapus Pemasukan?</DialogTitle>
          <DialogDescription className="text-center">
            Apakah Anda yakin ingin menghapus data pemasukan ini? Tindakan ini tidak dapat dibatalkan.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-3 text-sm my-4">
           <div className="flex justify-between items-center pb-2 border-b border-slate-200 border-dashed">
              ...
              <span className="font-semibold text-slate-800">{income.transaction_date ? format(new Date(income.transaction_date), 'dd/MM/yyyy') : '-'}</span>
           </div>
           <div className="flex justify-between items-center pb-2 border-b border-slate-200 border-dashed">
              <span className="text-slate-500 font-medium">Sumber</span>
              <span className="font-semibold text-slate-800">{income.sub_category || income.source}</span>
           </div>
           <div className="flex justify-between items-center">
              <span className="text-slate-500 font-medium">Jumlah</span>
              <span className="font-bold text-green-600 text-base">Rp {Number(income.amount).toLocaleString('id-ID')}</span>
           </div>
        </div>
...
export default AdminIncomeDeleteConfirmationModal;